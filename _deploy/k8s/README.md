...
| 镜像                      | 镜像端口    | 说明                 |
|-------------------------|-------|--------------------|
| noearorg/waterapi:2.13.0 | 9371  | 主接口服务              |
| noearorg/watersev:2.13.0       | 9372  | 批处理服务              |
| noearorg/wateradmin:2.13.0     | 9373  | 管理控制台              |
| noearorg/waterfaas:2.13.0      | 9374  | FaaS 即时接口服务        |
|                         |       |                    |
| noearorg/xwater:2.13.0         | 19371 | Water助理工具（仅在需要时启用） |
...
```shell
docker run -it --rm -p 19371:19371 noearorg/xwater:2.13.0
```
...
* 添加 water/waterapi 服务（镜像：noearorg/waterapi:2.13.0）。主接口
  * 镜像端口：9371
  * 对外端口：9371
  * 建议2个副本起步
...
* 添加 water/wateradmin 服务（镜像：noearorg/wateradmin:2.13.0）。管理控制台
  * 镜像端口：9373
  * 对外端口：9373 或其它
  * 建议1个副本即可
  * 要配置外网访问地址，建议加域名
...
* 添加 water/waterfaas 服务 （镜像：noearorg/waterfaas:2.13.0）。即时接口服务
  * 镜像端口：9374
  * 对外端口：9374 或其它
  * 建议1个副本起步
  * 要配置外网访问地址，建议加域名
...
* 添加 water/watersev-tol 服务（镜像：noearorg/watersev:2.13.0）。工具服务，包含： (msgchk,sevchk,mot,detapp,detssl)
  * 镜像端口：9372
  * 对外端口：9372 或其它
  * 建议1个副本起步
...
* 添加 water/watersev-pln 服务（镜像：noearorg/watersev:2.13.0）。定时任务调度服务
  * 镜像端口：9372
  * 对外端口：9372 或其它
...
* 添加 water/watersev-msgdis 服务（镜像：noearorg/watersev:2.13.0）。消息派发服务
  * 镜像端口：9372
  * 对外端口：9372 或其它
...
* 添加 water/watersev-msgexg 服务（镜像：noearorg/watersev:2.13.0）。消息交换服务
  * 镜像端口：9372
  * 对外端口：9372 或其它
