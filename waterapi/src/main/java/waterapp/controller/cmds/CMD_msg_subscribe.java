...
public class CMD_msg_subscribe extends CMDBase {
    ...
    private boolean do_subscprebe(String key,String note,String alarm_mobile, String topic, String receive_url, String access_key, int receive_way) throws Exception {
        ...
        if (DbWaterMsgApi.hasSubscriber(key, topic)) {
            DbWaterMsgApi.udpSubscriber(key, note, alarm_mobile, topic, receive_url, access_key, receive_way);
            return true;
        }

        if (DbWaterMsgApi.addSubscriber(key, note, alarm_mobile,topic, receive_url, access_key, receive_way) > 0) {
            return true;
        }
