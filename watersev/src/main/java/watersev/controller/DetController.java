...
public final class DetController implements IJob {
    ...
    private void exec0() throws SQLException {
        //取出待处理的服务（已启用的服务）
        List<ServiceModel> list = DbWaterDetApi.getServiceList();
    ...
    private void check_type1(ServiceModel sev) {
        ...
        if (seconds < 10) {
            //对签到型的服务进行检查
            //
            DbWaterDetApi.udpService1(sev.service_id, 0);
            ...
            if (sev.is_unstable && sev.check_error_num >= 2) {
                ...
                // 如果稳定服务，则提示出错
                //
                DbWaterDetApi.udpService1(sev.service_id, 1);
    ...
    private void check_type0_tcp(ServiceModel sev, String url) {
        ...
        try {
            ...
            DbWaterDetApi.setServiceState(sev.service_id, 1);//设为;正在处理中

            //ping 检测
            PingUtils.ping(uri.getAuthority(), 2000);

            DbWaterDetApi.udpService0(sev.service_id, 0, "");
            ...
            if (sev.is_unstable && sev.check_error_num >= 2) {
                ...
                DbWaterDetApi.udpService0(sev.service_id, 1, "0");
        ...
        try {
            ...
            HttpUtilEx.getStatusByAsync(url, (isOk, code, hint) -> {
                ...
                if (code >= 200 && code < 400) { //正常
                    DbWaterDetApi.udpService0(sev.service_id, 0, code + "");
                    ...
                    if (sev.is_unstable && sev.check_error_num >= 2 && !isOk) {
                        ...
                        DbWaterDetApi.udpService0(sev.service_id, 1, code + "");
                        LogUtil.sevWarn(getName(), sev.address, sev.name + "@" + sev.address + "\n" + url2 + ", " + hint);
            ...
            DbWaterDetApi.udpService0(sev.service_id, 1, ex.getMessage());
            LogUtil.sevWarn(getName(), sev.address, sev.name + "@" + sev.address + "\n" + Utils.throwableToString(ex));
        }
    }
}
