...
//import watersev.dso.etl.EtlTransformer;
//import watersev.models.water.PassEtlModel;
//
//import java.util.Date;
//import java.util.List;
//
//public class EtlTranController implements TaskUtil.ITask,EtlEngine.Handler {
//    @Override
...
//            }
//
//            while (task.t_max_instance > runList.num(task.etl_id)) {
...
//
//        AlarmUtil.tryNotice("ETL#" + cfg.get("name").getString() + "#转换器，已启动",task.alarm_mobile);
//
//        try {
//            new EtlEngine(new EtlTransformer(cfg))
//                    .handler(this)
//                    .start();
//        }catch (Exception ex){
//            ex.printStackTrace();
...
//        context.isPause = (task.t_enabled == 0);
//
//        if (context.isStop) {
//            runList.stop(context.id); //如果已停止，则从运行列表移徐
//            AlarmUtil.tryNotice("ETL#" + context.name + "#转换器，已停止",task.alarm_mobile);
//            return;
//        }
//
//        if (context.isPause) {
//            if(context.pause_state == 0) {
//                AlarmUtil.tryNotice("ETL#" + context.name + "#转换器，已暂停",task.alarm_mobile);
//            }
//
...
//
//            LogUtil.write("ETL",context.md5,new_md5);
//
//            if (context.md5.equals(new_md5) == false) {
//                AlarmUtil.tryNotice("ETL#" + context.name + "#转换器，开始更新配置",task.alarm_mobile);
//
//                ONode new_cfg = ONode.load(task.code);
//                new_cfg.set("md5", new_md5);
//
//                context.reloadConfig(new_cfg, EtlTransformer.class);
//                context.reloadFuns(EtlTransformer.class);
//
//                AlarmUtil.tryNotice("ETL#" + context.name + "#转换器，已重新加载配置",task.alarm_mobile);
//            }
//        }else{
//            if(context.pause_state == 1){
//                AlarmUtil.tryNotice("ETL#" + context.name + "#转换器，已重新启动",task.alarm_mobile);
//            }
//
//            context.pause_state = 0;
//
//            DbPaasApi.setEtlTransformTime(task.etl_id, new Date());
//        }
//    }
//}
