...
public class IDUtil {
    ...
    private static long buildID(String tag, long startIndex) throws Exception {
        ...
        return _redis.open1((ru) -> {
            return ru.key("ID").expire(60 * 60 * 24 * 365).hashIncr(tag, 1l);
        }) + startIndex;
    }
    ...
    private static void tryInit() {
        if (_redis == null) {
            ...
            try {
                ConfigM cfg = WaterClient.Config.get(Config.water_config_tag, "rock_redis");
                _redis = new RedisX(cfg.getProp(), 1);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
