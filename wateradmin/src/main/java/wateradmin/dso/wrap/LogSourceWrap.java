...
import org.noear.water.protocol.solution.LogSourceMongo;
import org.noear.water.protocol.solution.LogSourceRdb;

import java.util.Date;
import java.util.List;

public class LogSourceWrap implements LogSource {
    LogSource real;

    public LogSourceWrap(ConfigM cfg) {
        if (cfg.value.indexOf("=rdb") > 0) {
            real = new LogSourceRdb(cfg.getDb());
        } else {
